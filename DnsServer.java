...
public class DnsServer extends Thread {
	public DnsServer(int udpPort) throws UnknownHostException, SocketException {
		// create master resolver
		byte serverAddr[] = Denis.dnsSource.clone();
		//Original source for setting DNS address, replaced by above.
		//byte serverAddr[] = new byte[4];
		// FIXME: default dns server address. You could change it if you want
		//serverAddr[0] = (byte) 130;
		//serverAddr[1] = (byte) 108;
		//serverAddr[2] = (byte) 128;
		//serverAddr[3] = (byte) 200;
		...
		boolean boundPort = false, alternate = false;
		try {
		...
		//Attempt to bind the port. If this fails, use the alternate port.
		//Keep incrementing by one until it works.
		...
		while (!boundPort)
		{
			try {
				...
				System.out.println("Failed to bind port: " + udpPort);
				if (!alternate)
				{
					System.out.println("Trying alternate: " + Denis.fallbackPort);
					udpPort = Denis.fallbackPort;
					alternate = true;
				}
				else
				{
					System.out.println("Trying: " + ++udpPort);
				}
				continue;
			}
			boundPort = true;
		}
		System.out.println("Running on port: " + udpPort);
		initShellCommands();
	}
